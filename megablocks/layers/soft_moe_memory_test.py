...
from megablocks.layers import dmoe, soft_moe, arguments, mpu
...
def test_memory(
    ...
    args = arguments.Arguments(
        ...
        moe_top_k=top_k,
        moe_num_slots_per_expert= sequence_length // num_experts,
        moe_expert_model_parallelism=True,
        expert_parallel_group=group,
        fp16=True,
        bf16=False,
        device=torch.cuda.current_device())
    layer = soft_moe.SoftMoE(args).cuda()

    x = torch.randn(
        (sequence_length, batch_size, hidden_size),
        device=torch.cuda.current_device(),
        dtype=torch.float16).requires_grad_(True)
